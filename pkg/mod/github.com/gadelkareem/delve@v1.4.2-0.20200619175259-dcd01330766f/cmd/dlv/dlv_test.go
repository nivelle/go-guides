...
func getDlvBin(t *testing.T) (string, string) {
	...
	out, err := exec.Command("go", "build", "-o", dlvbin, "github.com/gadelkareem/delve/cmd/dlv").CombinedOutput()
	if err != nil {
		t.Fatalf("go build -o %v github.com/gadelkareem/delve/cmd/dlv: %v\n%s", dlvbin, err, string(out))
	}
...
func TestChildProcessExitWhenNoDebugInfo(t *testing.T) {
	if runtime.GOOS == "darwin" {
		t.Skip("test skipped on darwin, see https://github.com/gadelkareem/delve/pull/2018 for details")
	}
...
func TestTypecheckRPC(t *testing.T) {
	...
	pkgs, err := packages.Load(cfg, "github.com/gadelkareem/delve/service/rpc2")
	if err != nil {
		t.Fatal(err)
	}
	...
	packages.Visit(pkgs, func(pkg *packages.Package) bool {
		if pkg.PkgPath != "github.com/gadelkareem/delve/service/rpc2" {
			return true
		}
