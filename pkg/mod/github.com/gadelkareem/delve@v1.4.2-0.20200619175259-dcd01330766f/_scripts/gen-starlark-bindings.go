...
func genMapping(bindings []binding) []byte {
	...
	for _, binding := range bindings {
		...
		for i := range binding.argNames {
			...
			case "*github.com/gadelkareem/delve/service/api.LoadConfig":
			...
			case "*github.com/gadelkareem/delve/service/api.EvalScope":
...
func genDocs(bindings []binding) []byte {
	...
	for _, binding := range bindings {
		...
		fmt.Fprintf(&buf, "%s(%s) | Equivalent to API call [%s](https://godoc.org/github.com/gadelkareem/delve/service/rpc2#RPCServer.%s)\n", binding.name, argNames, binding.fn.Name(), binding.fn.Name())
	}
